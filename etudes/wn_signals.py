...
class WN_Signal_selec(object):
    ...
    """
    def __init__(self, psr=None, efac=True, equad=True, fix_wn=True,
                 fix_wn_vals=None, **kwargs):
        self.psr = psr
    ...
    def _init_fix_ndiag(self, efac=True, equad=True, fix_wn_vals=None):
        if efac and equad:
            self._ndiag = self.efacs**2 @ self.masks * (self.psr.toaerrs**2 + 10**(2 * self.equads) @ self.masks)
        elif efac and not equad:
