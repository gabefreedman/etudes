...
def make_model(psrs, has_rn=True, has_gwb=True, has_cw=True):
    ...
    if has_cw:
        ...
        params.extend(
            [cosinc, costheta, log10_Mc, log10_fgw, log10_h, phase0, phi, psi]
        )
...
#
def main(psrfile, resdir, nburnin=2_000, nsamples=10_000):
    """
    ...
    # load psr data from pickle file
    ...
    states = run_inference_algorithm(
        sample_key, state, kernel, nsamples, progress_bar=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("psrfile", type=str, help="pickle file containing pulsar data")
    parser.add_argument("resdir", type=str, help="directory for storing chains")
    parser.add_argument(
        "--nsamples",
        type=int,
        help="Number of NUTS samples to generate",
        default=10_000,
        required=False,
    )
    parser.add_argument(
        "--nburnin",
        type=int,
        help="Number of burnin adaptation samples for NUTS",
        default=2_000,
        required=False,
    )
